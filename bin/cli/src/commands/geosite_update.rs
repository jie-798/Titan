...
fn resolve_categories(config_path: &str, category: Option<&str>) -> anyhow::Result<Vec<String>> {
...
async fn download_category(base_url: &str, output_dir: &str, category: &str) -> anyhow::Result<()> {
    ...
    let output_path = Path::new(output_dir).join(format!("{}.txt", category));
